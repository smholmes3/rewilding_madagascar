...
filename = 'perch2_resample'
metadata = pd.read_csv('./data/metadata_mimer.csv')
...
# Next, resample the training labels to balance classes. We included n_samples_per_class as a variable at the top of the script, so you can change it there and it will apply here. If you want to downsample instead of upsample, change the downsample and upsample arguments accordingly. Note that if you have a class with fewer than n_samples_per_class examples, you won't be able to downsample to that number, so you may want to check your class distributions before deciding on the number of samples per class and whether to upsample or downsample.
